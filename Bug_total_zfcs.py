...
class CS():
    ...
    def total(self):
        ...
        cursor = conn.cursor()
        # sql语句
        sql_total = "SELECT count(*) FROM bugs INNER JOIN products ON bugs.product_id = products.id \
...
class SJ():
    ...
    def analysis(self):
        ...
        bar_width=0.3
        plt.bar(x = x_axis_1, height = y_axis_1, width=bar_width, color = '#1f77b4', alpha=0.8, label='问题数量')
        ...
        bar_width=0.3
        plt.bar(x = x_axis_2, height = y_axis_2, width=bar_width, color = '#d62728', alpha=0.8, label='问题数量')
        # 在柱状图上显示具体数值，ha参数控制水平对齐方式，va参数控制垂直对齐方式
...
layout = [
    ...
    [sg.Text('项目问题共计: ',font='微软雅黑',size=(10, 1)), sg.Text('', key='_total_',size=(10, 1))],
